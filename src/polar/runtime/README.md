...
- `ack/`: the whole remote-sandbox surface — both remote backends and all their
  docs, so the rest of the tree stays untouched. `ACKRuntime` (Kubernetes pods
  and OpenKruise sandbox pools, optional `ack` extra) is split across `_sdk.py`
  (extra guard and cluster constants), `_util.py` (manifest helpers),
  `client.py` (shared client manager) and `runtime.py` (the backend itself);
  `e2b.py` holds `E2BRuntime` (optional `e2b` extra), a pure client of the
  official `e2b` SDK that also drives a self-hosted E2B control plane.
  `README.md`, `RUNBOOK.md` and `QUICKSTART.md` live here too. See
...
For the command-first recipe on the ACK/ACS sandbox stack — sandbox components,
dataset conversion for both the HuggingFace and Harbor formats, required
variables, run and teardown — see [ack/QUICKSTART.md](ack/QUICKSTART.md).
...
[RUNBOOK.md](ack/RUNBOOK.md).

For the same deployment as a set of manifests — control pod, two `ClusterIP`
Services, warm sandbox pool, bootstrap ConfigMap, and a no-model probe that proves
the sandbox → gateway edge — see [ack/DEMO.md](ack/DEMO.md).

Remote backends also change *who has to reach whom*: the agent inside the sandbox
dials the gateway's `public_url` for every LLM call, so the URLs in
`topology.yaml` must be addresses that are visible across the cluster, and a
loopback value is a single-host test setting only. The per-edge addressing table,
the derived `127.0.0.1` defaults, and the symptom each broken edge produces are in
[network addressing](ack/RUNBOOK.md#network-addressing-who-must-reach-whom).

Everything added for remote sandboxes lives under `ack/`: both backends and all
the docs. `e2b.py` is not Kubernetes-specific — the same class also serves E2B's
own cloud, where there is no cluster at all — but a self-hosted E2B plane *is*
the ACK/ACS sandbox stack, so it sits with the rest of it. The `ack` package
re-exports lazily, which keeps the two extras independent: importing
`polar.runtime.ack.e2b` never pulls in the `kubernetes` SDK, and
`uv pip install 'polar[e2b]'` alone is enough for the `e2b` backend.
...
[E2B on a self-hosted control plane](ack/RUNBOOK.md#e2b-on-a-self-hosted-control-plane).
...
`SandboxSet` pool. Everything ACK-specific lives in [ack/README.md](ack/README.md):
allocation modes, RBAC, the `kwargs` table, `pod_overrides` and volume handling,
the OpenKruise claim lifecycle, pool sizing, running the shipped examples,
teardown semantics and troubleshooting.
...
mounted tooling without any error; bake it into the image or install it in
`prepare`.
...
the runbook's
[One dataset, many images](ack/RUNBOOK.md#one-dataset-many-images).
